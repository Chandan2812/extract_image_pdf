...
var PDFPage = /** @class */ (function (_super) {
    ...
    function PDFPage() {
        ...
        _this.normalizeContents = function () {
            ...
            if (actualContents instanceof pdf_objects_1.PDFIndirectReference) {
                var lookedUpContents = _this.index.lookup(actualContents);
                if (lookedUpContents instanceof pdf_objects_1.PDFArray) {
                    _this.set('Contents', lookedUpContents.clone());
                }
                else {
                    _this.set('Contents', pdf_objects_1.PDFArray.fromArray([actualContents], _this.index));
                }
            }
            return _this;
        };
        ...
        _this.normalizeResources = function (_a) {
            ...
            if (!_this.getMaybe('Resources')) {
                _this.set('Resources', pdf_objects_1.PDFDictionary.from(new Map(), _this.index));
            }
            if (Font && !_this.Resources.getMaybe('Font')) {
                _this.Resources.set('Font', pdf_objects_1.PDFDictionary.from(new Map(), _this.index));
            }
            if (XObject && !_this.Resources.getMaybe('XObject')) {
                _this.Resources.set('XObject', pdf_objects_1.PDFDictionary.from(new Map(), _this.index));
            }
        };
        ...
        _this.addContentStreams = function () {
            ...
            validate_1.validateArr(contentStreams, validate_1.isInstance(pdf_objects_1.PDFIndirectReference), '"contentStream" must be of type PDFIndirectReference<PDFContentStream>');
            _this.normalizeContents();
            if (_this.autoNormalizeCTM)
                _this.normalizeCTM();
            if (!_this.getMaybe('Contents')) {
                _this.set('Contents', pdf_objects_1.PDFArray.fromArray(contentStreams, _this.index));
            }
            else {
        ...
        fontDict) {
            validate_1.validate(key, isString_1.default, '"key" must be a string');
            validate_1.validate(fontDict, validate_1.isInstance(pdf_objects_1.PDFIndirectReference), '"fontDict" must be an instance of PDFIndirectReference');
        ...
        _this.addXObject = function (key, xObject) {
            validate_1.validate(key, isString_1.default, '"key" must be a string');
            validate_1.validate(xObject, validate_1.isInstance(pdf_objects_1.PDFIndirectReference), '"xObject" must be an instance of PDFIndirectReference');
        ...
        _this.clone = function () {
            return PDFPage.fromDict(pdf_objects_1.PDFDictionary.from(new Map(_this.map), _this.index));
        };
        return _this;
    }
    ...
    PDFPage.create = function (index, size, resources) {
        validate_1.validate(size, isArray_1.default, 'size must be an array of 2 numbers.');
        validate_1.validate(size.length, validate_1.isIdentity(2), 'size tuple must have two elements.');
        validate_1.validate(size[0], isNumber_1.default, 'size tuple entries must be Numbers.');
        validate_1.validate(size[1], isNumber_1.default, 'size tuple entries must be Numbers.');
        validate_1.validate(resources, validate_1.optional(validate_1.isInstance(pdf_objects_1.PDFDictionary)), 'resources must be a PDFDictionary');
        var mediaBox = [0, 0, size[0], size[1]];
        var page = new PDFPage({
            Type: pdf_objects_1.PDFName.from('Page'),
            // TODO: Convert this to use PDFRectangle
            MediaBox: pdf_objects_1.PDFArray.fromArray(mediaBox.map(pdf_objects_1.PDFNumber.fromNumber), index),
        }, index, VALID_KEYS);
        if (resources)
